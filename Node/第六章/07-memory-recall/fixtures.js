...
export const now = new Date('2026-09-06T12:00:00+08:00')
export const question = '请按我以前的习惯，简单说说这次咖啡机退款需要人工审核吗，准备材料时有什么要注意的？现在还是按以前的标准处理吗？'

// 模拟 Checkpointer 已恢复的当前 Thread；不是从长期记忆猜测本次订单。
export const threadState = {
	summary: '本次只咨询退款流程，用户尚未提交退款申请。',
	messages: [
		{ role: 'user', content: '这次是订单 A2026，咖啡机退款金额 3500 元。' },
		{ role: 'assistant', content: '好的，我会核对本次退款要求。' }
	]
}
...
export const currentFacts = {
	tenantId: 'bluewhale',
	userId: 'user-1001',
	order: { id: 'A2026', refundAmount: 3500 },
	policy: {
		id: 'refund-policy',
		version: 3,
		status: 'active',
		...
		content: '退款金额超过 2000 元时必须进入人工审核。是否最终退款，还需核验订单的其他退款条件。'
	}
}

function source(kind, id) {
	return { kind, id, observedAt: '2026-09-01T09:00:00+08:00' }
}

export const profiles = [
	{ key: 'response_language', value: 'zh-CN' },
	{ key: 'answer_style', value: 'conclusion_first' },
	{ key: 'preferred_runtime', value: 'Node.js' }
].map((profile) => ({
	...profile,
	expiresAt: null,
	revision: 1,
...
export const events = [
	{
		...
		content: '上次咖啡机退款申请，因为缺少机器序列号照片被退回补充。用户补交照片后，申请进入人工审核。',
		status: 'active',
	...
	{
		key: 'expired-contact',
		content: '办理咖啡机退款时，用户出差期间只能通过邮件联系；这个要求到 9 月 5 日结束为止。',
		status: 'active',
	...
	},
	{
		...
		content: '上次咖啡机退款被退回，是因为用户没有提交发票照片。此原因后来已修正为缺少机器序列号照片。',
		status: 'superseded',
...
].map((event) => ({ expiresAt: null, revision: 1, ...event }))
